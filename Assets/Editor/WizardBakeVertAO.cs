      ...
      for (i=0; i<l; i++) {
        ...
        for (j=0; j<samples; j++) {
          ...
          if (debugSample) {
            ...
            if (isHit)
              sphere = (GameObject) PrefabUtility.InstantiatePrefab( psphere_hit );
            else
              sphere = (GameObject) PrefabUtility.InstantiatePrefab( psphere_miss );
            ...
            sphere = (GameObject) PrefabUtility.InstantiatePrefab( psphere );
            sphere.name += "_"+sample;
            sp = offset;
